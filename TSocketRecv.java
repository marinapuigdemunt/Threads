...
public class TSocketRecv extends TSocket_base {
  ...
  public int receiveData(byte[] buf, int offset, int length){
    //offset s'actualitza
    //length byte q falten per ficar al buf
    lock.lock();
    int constotal=0,cons=0;
    try{
        while(rcvQueue.empty()){appCV.awaitUninterruptibly();} 
        while(constotal<length){
        cons=consumeSegment(buf,offset,length);
        offset+=cons;
        constotal+=cons;
     }   
    }finally{
  ...
  public void processReceivedSegment(TCPSegment rseg) {  
    ...
    try{
      if(!rcvQueue.full()){
          ...
          System.out.println("S'ha descartat");
      }
    }finally{
      lock.unlock();
    }
